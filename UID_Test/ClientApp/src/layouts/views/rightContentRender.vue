  ...
  changeUserMenu(event) {
    switch (event.key) {
      case this.$locales.action_user_codeGenerator:
        this.$router.push({ name: 'webview', query: { src: encodeURIComponent('http://localhost:25124/_codegen?ui=vue3'), name: this.$locales.action_user_codeGenerator } })
        break;
      case this.$locales.action_user_apiDocument:
        this.$router.push({ name: 'webview', query: { src: encodeURIComponent('http://localhost:25124/swagger/index.html'), name: this.$locales.action_user_apiDocument } })
        break;
