...
<template>
  <WtmDetails :queryKey="queryKey" :loading="Entities.loading" :onFinish="onFinish">
    <a-row :gutter="6">
      <a-col :span="12">
         <WtmField entityKey="Imp_lnoAdd"/>
      </a-col>
      <a-col :span="12">
         <WtmField entityKey="VendoridAdd"/>
      </a-col>
      <a-col :span="12">
         <WtmField entityKey="Card_NoAdd"/>
      </a-col>
      <a-col :span="12">
         <WtmField entityKey="Cust_PartNoAdd"/>
      </a-col>
    </a-row>

      <div>
        <WtmField entityKey="Detail_MasterAdd" :fieldProps="{ columnDefs:DetailMasterListcolumnDefs,gridOptions:DetailMasterListgridOptions }" />
      </div>
...
export default class extends mixins(PageDetailsBasics) {
  ...
  get DetailMasterListcolumnDefs(): (ColDef | ColGroupDef)[] {
    return [
      { field: "Seq", headerName: "Page序號" },
      { field: "If_Repeated", headerName: "Page是否重複" ,cellRenderer: this.$FrameworkComponents.switch},
      { field: "Repeate_No", headerName: "Page重複次數" },
    ];
  }

  get DetailMasterListgridOptions(): GridOptions {
    return {};
  }
